...
describe('NoteService', () => {
    ...
    it('when try insert with author then do it', async () => {
        ...
        await service.insert(noteInsert, noteInsert.author)

        expect(userQueryServiceMock.findById).toHaveBeenCalledWith(noteInsert.author, noteInsert.author)
        expect(noteModelMock.create).toHaveBeenCalledWith({ ...noteInsert })
    })

    it('when try insert with another author then throw error', async () => {
        const noteInsert = noteInsertFactory.build()
        const author = userDetailFactory.build()
        const model = noteDocumentFactory.build() as any
        ...
        userQueryServiceMock.findById!.mockResolvedValue(author)
        noteModelMock.create!.mockResolvedValue(model)
        await expect(service.insert(noteInsert, anotherAuthorId)).rejects.toThrow(UnauthorizedException)

        expect(userQueryServiceMock.findById).not.toHaveBeenCalled()
        expect(noteModelMock.create).not.toHaveBeenCalled()
    })
    ...
    it('when try insert with non stored author then throw error', async () => {
        ...
        await expect(service.insert(noteInsert, noteInsert.author)).rejects.toThrow(NotFoundException)
    ...
    it('when try update stored note then return it', async () => {
        ...
        await expect(service.update(noteUpdate, noteUpdate.author)).resolves.not.toThrow()

        expect(queryServiceMock.findById).toHaveBeenCalledWith(noteUpdate.id, noteUpdate.author, noteUpdate.author)
    ...
    it('when try update stored note belongs to another author then throw error', async () => {
        const noteUpdate = noteUpdateFactory.build()
        const founded = noteDetailFactory.build()
        const updated = noteDocumentFactory.build() as NoteDocument
        const anotherAuthorId = new ObjectId().toHexString()
        ...
        await expect(service.update(noteUpdate, anotherAuthorId)).rejects.toThrow(UnauthorizedException)

        expect(queryServiceMock.findById).not.toHaveBeenCalled()
        expect(noteModelMock.findOneAndUpdate).not.toHaveBeenCalled()
    ...
    it('when try update note not found then throw error', async () => {
        ...
        await expect(service.update(noteUpdate, noteUpdate.author)).rejects.toThrow(NotFoundException)
        expect(queryServiceMock.findById).toHaveBeenCalledWith(noteUpdate.id, noteUpdate.author, noteUpdate.author)
    ...
    it('when note is deleted then not throw error', async () => {
        ...
        await expect(service.delete(id, authorId, authorId)).resolves.not.toThrow()
        expect(queryServiceMock.findById).toHaveBeenCalledWith(id, authorId, authorId)
        expect(noteModelMock.findByIdAndDelete).toHaveBeenCalledWith(id)
    })

    it('when note to be delete belongs to another author then throw error', async () => {
        const id = new ObjectId().toHexString()
        const authorId = new ObjectId().toHexString()
        const noteDetail = noteDetailFactory.build()
        const anotherAuthorId = new ObjectId().toHexString()
        ...
        await expect(service.delete(id, authorId, anotherAuthorId)).rejects.toThrow(UnauthorizedException)
        expect(queryServiceMock.findById).not.toHaveBeenCalled()
    ...
    it('when note not found then throw error', async () => {
        ...
        await expect(service.delete(id, authorId, authorId)).rejects.toThrow(NotFoundException)
        expect(queryServiceMock.findById).toHaveBeenCalledWith(id, authorId, authorId)
        expect(noteModelMock.findByIdAndDelete).not.toHaveBeenCalled()
    })
})